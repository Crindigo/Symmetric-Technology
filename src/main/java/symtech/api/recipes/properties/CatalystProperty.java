...
public class CatalystProperty extends RecipeProperty<CatalystPropertyValue> {
    ...
    public void drawInfo(@NotNull Minecraft minecraft, int x, int y, int color, Object value) {
        ...
        String localisedCatalystGroupName = I18n.format("symtech.catalyst_group." + propertyValue.getCatalystGroup().getName() + ".name");
        if(tier == CatalystInfo.NO_TIER) {
            minecraft.fontRenderer.drawString(I18n.format("symtech.recipe.catalyst", localisedCatalystGroupName, ""), x, y, color);
        } else {
            minecraft.fontRenderer.drawString(I18n.format("symtech.recipe.catalyst", GTValues.VN[tier], localisedCatalystGroupName), x, y, color);
        }
    }
}
