package symtech.api.recipes.builders;
...
public class MixerSettlerRecipeBuilder extends RecipeBuilder<MixerSettlerRecipeBuilder> {
    ...
    public MixerSettlerRecipeBuilder requiredCells(int cells) {
        if (cells <= 0) {
            SymtechLog.logger.error("Required mixer settler cell count cannot be less than or equal to one."
                    , new IllegalArgumentException());
            recipeStatus = EnumValidationResult.INVALID;
        }

        if (cells % 2 != 0) {
            SymtechLog.logger.error("Required mixer settler cell count must be even."
                    , new IllegalArgumentException());
            recipeStatus = EnumValidationResult.INVALID;
        }
