package symtech.api.metatileentity.multiblock;
 ...
 */
public class SymtechPredicates {
    ...
    private static final Supplier<TraceabilityPredicate> COOLING_COILS = () -> new TraceabilityPredicate(blockWorldState -> {
        ...
        if (state.getBlock() instanceof BlockCoolingCoil) {
            BlockCoolingCoil.CoolingCoilType type = SymtechBlocks.COOLING_COIL.getState(state);
        ...
        return false;
            ...
            .map(type -> new BlockInfo(SymtechBlocks.COOLING_COIL.getState(type)))
    ...
    private static final Supplier<TraceabilityPredicate> SINTERING_BRICKS = () -> new TraceabilityPredicate(blockWorldState -> {
        ...
        if (state.getBlock() instanceof BlockSinteringBrick) {
            BlockSinteringBrick.SinteringBrickType type = SymtechBlocks.SINTERING_BRICK.getState(state);
            Object currentBrick = blockWorldState.getMatchContext().getOrPut("SinteringBrickType", type);
            if (!currentBrick.equals(type)) {
                blockWorldState.setError(new PatternStringError("symtech.multiblock.pattern.error.sintering_bricks"));
                return false;
            }
            blockWorldState.getMatchContext().getOrPut("VABlock", new LinkedList<>()).add(blockWorldState.getPos());
            return true;
        ...
        return false;
            ...
            .map(type -> new BlockInfo(SymtechBlocks.SINTERING_BRICK.getState(type)))
            .toArray(BlockInfo[]::new)
    );
                    ...
                    facing -> () -> new TraceabilityPredicate(blockWorldState -> {
                        ...
                        if (state.getBlock() instanceof BlockConveyor) {
                            ...
                            if (!currentConveyor.equals(type)) {
                                blockWorldState.setError(new PatternStringError("symtech.multiblock.pattern.error.conveyor"));
                                return false;
                            }
                        ...
                        return false;
                            ...
                            .map(entry -> new BlockInfo(SymtechBlocks.CONVEYOR_BELT.getState(entry), null))
                            .toArray(BlockInfo[]::new)
                    ).addTooltips("symtech.multiblock.pattern.error.conveyor")));

    public static Supplier<TraceabilityPredicate> COILS_OR_BED = () -> new TraceabilityPredicate(blockWorldState -> {
        IBlockState blockState = blockWorldState.getBlockState();
        if (GregTechAPI.HEATING_COILS.containsKey(blockState) || blockState == SymtechBlocks.EVAPORATION_BED.getDefaultState()) {
            ...
            if (!current.equals(key)) {
                blockWorldState.setError(new PatternStringError("symtech.multiblock.pattern.error.coils_or_bed"));
                return false;
            }
            blockWorldState.getMatchContext().getOrPut("VABlock", new LinkedList<>()).add(blockWorldState.getPos());
            return true;
        ...
        return false;
            ...
            .toArray(BlockInfo[]::new))
            .addTooltips("symtech.multiblock.pattern.error.coils_or_bed");

    private static final Supplier<TraceabilityPredicate> EVAP_BED = () -> new TraceabilityPredicate(blockWorldState -> false, () ->
            new BlockInfo[]{new BlockInfo(SymtechBlocks.EVAPORATION_BED.getDefaultState())})
            .addTooltips("symtech.multiblock.pattern.error.coils_or_bed");
