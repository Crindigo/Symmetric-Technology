...
public class FiberProperty implements IMaterialProperty {
    ...
    public void verifyProperty(MaterialProperties properties) {
        ...
        if (this.solutionSpun) { properties.getMaterial().addFlags(SymtechMaterialFlags.GENERATE_WET_FIBER); }
        if (this.weaving) { properties.getMaterial().addFlags(MaterialFlags.GENERATE_PLATE); }

        properties.getMaterial().addFlags(SymtechMaterialFlags.GENERATE_FIBER, SymtechMaterialFlags.GENERATE_THREAD, MaterialFlags.DISABLE_DECOMPOSITION);
    }
}
