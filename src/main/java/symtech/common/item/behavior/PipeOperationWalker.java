package symtech.common.item.behavior;
...
public class PipeOperationWalker<T extends IPipeTile<?, ?>> {
    ...
    private void traverse(int maxWalks) {
        ...
        if (walkedBlocks >= maxWalks) {
            SymtechLog.logger.fatal("The walker reached the maximum amount of walks {}", walkedBlocks);
        }
        invalid = true;
    }
    ...
    private boolean checkCurrent() {
        ...
        if (currentPipe == null) {
            ...
            if (!(thisPipe instanceof IPipeTile)) {
                SymtechLog.logger.fatal("PipeWalker expected a pipe, but found {} at {}", thisPipe, currentPos);
                return false;
            }
